...
\begin{document}
\graphicspath{{images/}}
\history{Date of publication April 03, 2026, date of current version April 03, 2026.}
\doi{10.1109/ACCESS.2024.0429000}

\title{Autonomic Calibration for Iatrogenic Error Mitigation in Industrial Vibration Monitoring Systems}
\author{\uppercase{Ivan Fernando Vazquez-Lopez}\authorrefmark{1}}

\address[1]{Universidad de Sonora, Hermosillo, SON 83000 MEX (e-mail: a216201216@unison.mx)}

\tfootnote{This work was made as a final exam for the Autonomic Computing subject from the Master of Engineering in Internet of Things and Artificial Intelligence program, at the Universidad de Sonora.}
...
\begin{abstract}
This work presents an autonomic computing approach based on a feedback control approach known as MAPE-K (Monitor, Analyze, Plan, and Execute over a shared Knowledge) to mitigate iatrogenic errors in industrial vibration monitoring. In precision maintenance, sensor misalignment during re-installation can indicate a false positive diagnosis of mechanical failure. To address this, we have implemented a Zero-shot vector decomposition algorithm to autonomously detect and correct any sensor tilt that may occur as a result of normal operation. By utilizing centrifugal force principles (\(F_c \propto \omega^2\)) as a shared Knowledge base, the system automatically distinguishes between mechanical failure and sensor shifts. Experimental results demonstrate that the MAPE- K loop detected a tilt induced by an incorrect treseating of the sensor.
\end{abstract}

\begin{keywords}
Autonomic computing, edge computing, iatrogenesis, industrial mechatronics, MAPE-K loop, physics-informed modeling, rotation matrices, self-healing systems, sensor misalignment, vibration, zero-shot.
analysis.
\end{keywords}
...
\PARstart{T}{he} Monitor, Analyze, Plan, Execute over a shared Knowledge (MAPE-K) loop is a widely used autonomic computing framework for self-managing systems.
